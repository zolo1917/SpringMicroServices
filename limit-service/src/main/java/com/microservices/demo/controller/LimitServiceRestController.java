...
public class LimitServiceRestController {
	...
	public ZipCodeModel getzipStateDetails(@PathVariable Long zipCode) {
		...
		String url = "http://" + instanceInfo.getIPAddr() + ":" + instanceInfo.getPort() + "/" + "/getCityState/"
				+ zipCode.toString();

		System.out.println("URL" + url);
		ZipCodeModel zip = this.restTemplate.getForObject("http://localhost:8001/getCityState/{zipCode}",
				ZipCodeModel.class, zipCode);
		return zip;
	}
}
